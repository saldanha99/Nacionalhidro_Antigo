...
import { buscarVendedores } from "@src/redux/actions/administrador/vendedor/buscarVendedoresActions"
import { cadastrarVendedor } from "@src/redux/actions/administrador/vendedor/cadastrarVendedorActions"
import { alterarVendedor } from "@src/redux/actions/administrador/vendedor/alterarVendedorActions"
...
const Vendedor = (props) => {
  ...
  const save = (data) => {
    ...
    swal({
      ...
      .then((ok) => {
        if (ok) {
          handleClose()
          if (data.id === 0) props.cadastrarVendedor({ data })
          else props.alterarVendedor({ data })
  ...
  return (
    <Card>
      ...
      <ModalVendedor data={data} modal={modal} handleClose={handleClose} save={save} />
      {
        !loadingSkeleton
        && <DataTable
          noHeader
...
const mapStateToProps = (state) => {
  return {
    lista: state?.vendedor?.lista,
    isFinishedAction: state?.vendedor?.isFinishedAction,
    error: state?.vendedor?.error
  }
}
