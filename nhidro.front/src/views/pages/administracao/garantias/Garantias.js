...
import { buscarGarantias } from "@src/redux/actions/administrador/garantia/buscarGarantiaActions"
import { cadastrarGarantia } from "@src/redux/actions/administrador/garantia/cadastrarGarantiaActions"
import { alterarGarantia } from "@src/redux/actions/administrador/garantia/alterarGarantiaActions"
...
const Garantias = (props) => {
  ...
  const save = (data) => {
    ...
    swal({
      ...
      .then((ok) => {
        if (ok) {
          handleClose()
          if (data.id === 0) props.cadastrarGarantia({ data })
          else props.alterarGarantia({ data })
        } else swal("Processo cancelado!")
      })
  }

  useEffect(() => {
    props.buscarGarantias()
  ...
  return (
    <Card>
      ...
      <ModalGarantia data={data} modal={modal} handleClose={handleClose} save={save} />
      {
        !loadingSkeleton
        && <DataTable
          noHeader
...
const mapStateToProps = (state) => {
  return {
    lista: state?.garantia?.lista,
    isFinishedAction: state?.garantia?.isFinishedAction,
    error: state?.garantia?.error
  }
}
