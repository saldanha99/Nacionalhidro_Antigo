...
import { cadastrarCentroCusto } from "@src/redux/actions/administrador/centro-custo/cadastrarCentroCustoActions"
import { alterarCentroCusto } from "@src/redux/actions/administrador/centro-custo/alterarCentroCustoActions"
...
const CentroCusto = (props) => {
  ...
  const save = (data) => {
    ...
    .then((ok) => {
      if (ok) {
        handleClose()
        if (data.id === 0) props.cadastrarCentroCusto({data})
        else props.alterarCentroCusto({data})
  ...
  return (
        <Card>
          ...
          <ModalCentroCusto data={data} modal={modal} handleClose={handleClose} save={save} />
          {
            !loadingSkeleton
            && <DataTable
                noHeader
...
const mapStateToProps = (state) => {
  return {
    lista: state?.centro?.lista,
    isFinishedAction: state?.centro?.isFinishedAction,
    error: state?.centro?.error
  }
}
