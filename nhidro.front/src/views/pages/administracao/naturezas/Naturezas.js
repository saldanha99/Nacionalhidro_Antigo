...
import { cadastrarNatureza } from "@src/redux/actions/administrador/natureza-contabil/cadastrarNaturezaActions"
import { alterarNatureza } from "@src/redux/actions/administrador/natureza-contabil/alterarNaturezaActions"
import { ChevronDown, ChevronLeft, ChevronRight } from "react-feather"
import { FiEdit2} from 'react-icons/fi'
import ModalNaturezas from './ModalNaturezas'
import {ToastContent } from "@utils"
import { toast, Slide } from "react-toastify"
import swal from 'sweetalert'

const Naturezas = (props) => {
  ...
  const handleToastSuccess = () => {
    toast.success(
      <ToastContent
        messageTitle="Natureza Contábil"
  ...
  const save = (data) => {
    ...
    .then((ok) => {
      if (ok) {
        handleClose()
        if (data.id === 0) props.cadastrarNatureza({data})
        else props.alterarNatureza({data})
  ...
  return (
        <Card>
          ...
          <ModalNaturezas data={data} modal={modal} handleClose={handleClose} save={save} />
          {
            !loadingSkeleton
            && <DataTable
                noHeader
...
const mapStateToProps = (state) => {
  return {
    lista: state?.natureza?.lista,
    isFinishedAction: state?.natureza?.isFinishedAction,
