...
const AbaDocumentos = (props) => {
  ...
  const columns = [
    {
      ...
      Cell: (row) => {
        return (
          <div>
            {user.role.name.includes('Gerencial') && <FiX title="Deletar" style={{ margin: "5px" }} size={20} onClick={() => deletar(row.original)} />}
            <FiDownload
              title="Download"
              style={{ margin: "5px" }}
              size={20}
  ...
  useEffect(() => {
    if (fileId) {
      ...
      cadastrarClienteDocumento({ data });
      setModel(null);
    }
  }, [fileId]);
