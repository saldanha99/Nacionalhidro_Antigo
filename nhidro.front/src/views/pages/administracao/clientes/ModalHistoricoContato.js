...
const ModalHistoricoContato = (props) => {
  const { data, modal, setModal, cliente, vendedor, vendedores, save } = props
  ...
  useEffect(() => {
    if (modal) {
      if (data) {
        ...
        setState({...state, Cliente: cliente, Vendedor: vendedor.role.name !== 'Gerencial' ? vendedor : {}, Contato: {}, DataContato: moment().format('YYYY-MM-DD'), DataAgendado: '', Observacoes: '', Status: 'NOVO'})
      }
    }
  }, [modal])
        ...
        >
              ...
              <Col md="6" className="mt-1">
                <FormGroup>
                  ...
                  <Select
                      ...
                      name="Contato"
                      noOptionsMessage={() => 'Sem registro!'}
                      options={cliente?.Contatos}
                      isSearchable
                      getOptionLabel={(option) => `${option?.Nome}-${option.Setor}`}
                      getOptionValue={(option) => option}
                      value={
                        cliente?.Contatos?.filter((option) => option.id === state.Contato?.id)
                      }
                      onChange={(object) => {
          ...
          <ModalFooter>
            <Button.Ripple
              color="primary"
              className="mr-1 mb-1"
              disabled={!state.Vendedor?.id || !state.Contato?.id || !state.DataContato}
