...
import { buscarClientesporVendedor } from "@src/redux/actions/administrador/cliente/listaClientesActions"
import { buscarHistoricoContato, cadastrarHistoricoContato, alterarHistoricoContato } from "../../../../redux/actions/comercial/historico-contato/buscarHistoricoActions"
import ModalHistoricoContato from "./modals/ModalHistoricoContato"
...
const vendedor = auth.getUserInfo()
...
const HistoricoContato = (props) => {
  ...
  useEffect(() => {
    setLoading(true)
    props.buscarClientesporVendedor(vendedor)
    props.buscarVendedores()
    props.buscarHistoricoContato()
  }, [])

  useEffectAfterMount(() => {
    setLoading(false)
    const items = props.historicoContato?.map(x => {
      return {
        Historico: x,
        Cliente: x.Cliente?.RazaoSocial ?? x.DescricaoCliente.toUpperCase(),
        Vendedor: x.Vendedor?.username,
        Contato: x.Cliente?.id ? `${x.Contato?.Nome} - ${x.Contato?.Setor}` : '',
        Status: x.Status,
  ...
  useEffectAfterMount(() => {
    ...
    props.buscarHistoricoContato(vendedor)
    ...
    MySwal.fire({
      ...
      .then((ok) => {
        if (ok.value) {
          ...
          setModal(false)
          setData(null)
          if (!data.DataAgendado) delete data.DataAgendado
          if (!data.id) {
            props.cadastrarHistoricoContato({ data })
          } else {
            props.alterarHistoricoContato({ data })
          }
        }
      })
  }
  ...
  return (
    <Card>
        <ModalHistoricoContato data={data} modal={modal} setModal={setModal} vendedor={vendedor} vendedores={props.vendedores} clientes={props.clientes} save={save} />
        <Row className="justify-content-between mt-5 mb-5 ml-2 mr-2">
            <Col style={{ textAlign: 'right' }}>
                ...
                {' '}
                <Button onClick={() => handleExportToExcel(state?.filteredData, `Relatorio_Historico_Contatos`, columns)} color='secondary'>
                Exportar
                </Button>
                {' '}
                <Button onClick={() => { 
