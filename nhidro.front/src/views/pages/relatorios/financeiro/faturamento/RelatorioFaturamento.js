...
import { buscarFaturamentosRelatorio } from "@src/redux/actions/financeiro/faturamento"
...
const RelatorioFaturamento = (props) => {
  ...
  const handleExportToExcel = () => {
    exportToExcel(groupObjByColumns(), `Relatorio_Faturamento_${moment(new Date()).utc().format("YYYYMMDDhmmss")}`)
  }
  ...
  const handleFiltrarBtn = (relatorio, intervaloData) => {
    if (intervaloData.length && intervaloData[0] && intervaloData[1]) {
      ...
      props.buscarFaturamentosRelatorio(intervaloData[0], intervaloData[1])
      setFilteredReactTable([])
  ...
  useEffectAfterMount(() => {
    if (props.relatorio?.length) {
      props.relatorio.forEach(value => {
        value.dias_aberto = DiffDatesInDays(value.status === 'Enviado' || value.status === 'Emitido' ? moment(value.data_emissao_aux) : value.status === 'Cancelada' ? moment(value.data_cancelamento_aux) : moment(), moment(value.data_aprovacao_aux))
      })
    }
...
const mapStateToProps = (state) => {
  return {
    relatorio: state?.faturamento?.relatorio,
    error: state?.faturamento?.error
  }
}

export default connect(mapStateToProps, {
  buscarFaturamentosRelatorio
