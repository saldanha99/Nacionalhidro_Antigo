...
import { buscarRelatorioConta } from "@src/redux/actions/financeiro/contas-a-pagar/relatorioContaActions"
...
const RelatorioContasAPagar = (props) => {
  ...
  const relatorios = [
    { label: 'Relatório de Contas a Pagar Simplificado', value: 'relatorio-simplificado' },
    { label: 'Relatório de Contas a Pagar Detalhado', value: 'relatorio-detalhado' },
    { label: 'Relatório de Contas Pagas', value: 'relatorio-pagas-periodo' },
    { label: 'Relatório de Contas por Competencia', value: 'relatorio-competencia' },
    { label: 'Relatório de Contas por Centro de Custo', value: 'relatorio-competencia-centro' },
    { label: 'Relatório de Contas por Natureza Contábil', value: 'relatorio-competencia-natureza' },
    { label: 'Relatório de Contas a Pagar em Atraso', value: 'relatorio-atraso' }
  ]
  ...
  const handleExportToExcel = () => {
    exportToExcel(groupObjByColumns(), `Relatorio_Contas_A_Pagar_${moment(new Date()).utc().format("YYYYMMDDhmmss")}`)
  }
  ...
  const handleFiltrarBtn = (relatorio, intervaloData) => {
    if (intervaloData.length && intervaloData[0] && intervaloData[1]) {
      ...
      props.buscarRelatorioConta({ relatorio, dates: intervaloData })
      setFilteredReactTable([])
  ...
  useEffectAfterMount(() => {
    const dataFormatada = props?.relatorio?.map(item => {
      item.DataEmissaoNF = item.DataEmissaoNF === '-' ? '-' : moment(item.DataEmissaoNF).local().format('DD/MM/YYYY')
      item.Vencimento = item.Vencimento === '-' ? '-' : moment(item.Vencimento).local().format('DD/MM/YYYY')
      item.VencimentoReal = item.VencimentoReal === '-' ? '-' : moment(item.VencimentoReal).local().format('DD/MM/YYYY')
      return item
    })
    setState({ ...state, data: dataFormatada, filteredData: dataFormatada })
